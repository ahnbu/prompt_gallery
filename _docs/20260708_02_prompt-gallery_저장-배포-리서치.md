...
쉽게 말하면, 앱 화면 파일(Cloudflare 용어로 **Static Assets**: HTML, CSS, JS, 아이콘, 기본 이미지)은 Cloudflare가 그대로 보내고, 저장·수정·삭제 요청(**Worker API**)은 Worker가 처리한다. 프롬프트·태그·Workflow·즐겨찾기 같은 목록표 데이터(**D1**)는 D1에 저장하고, 사용자가 업로드한 JPG/PNG 썸네일 이미지(**R2 object**)는 R2에 저장한다.

- **Workers + Static Assets**: 앱 화면 파일(HTML, CSS, JS, 아이콘)을 보내는 일과 저장·조회 API를 같은 Cloudflare Workers 런타임 안에서 다룬다.
- **Vite plugin**: 로컬 개발 때 앱 화면과 저장·조회 API(Worker API), Cloudflare 저장소 연결(binding)을 production과 가깝게 확인한다.
- **D1**: 프롬프트, 이미지 프롬프트, Workflow, 레포, 태그, 자동 분류 키워드, 즐겨찾기 같은 목록표 데이터(database)를 저장한다.
- **R2**: 사용자가 웹사이트에서 업로드하는 JPG/PNG preview 파일(object)을 저장한다.
...
D1은 엑셀 표처럼 검색·필터·수정할 데이터(database)를 저장하는 곳이고, R2는 이미지 파일함처럼 JPG/PNG 같은 파일(object)을 저장하는 곳이다. D1에는 이미지 파일 자체를 넣지 않고, “R2의 어느 위치에 이미지가 있는지”를 가리키는 값(`image_key`)만 저장한다.

이미지 프롬프트 저장 흐름:

```markdown
1. 사용자가 프롬프트를 입력한다.
2. 내 PC에서 JPG/PNG 썸네일을 선택한다.
3. Worker API가 이미지 파일(R2 object)은 R2에 저장한다.
4. Worker API가 제목, 본문, 태그, 즐겨찾기 여부, 이미지 위치(image_key)는 D1에 저장한다.
5. 카드 목록은 D1의 목록표 데이터를 읽고, 썸네일은 image_key를 따라 R2에서 불러온다.
```

라우팅 원칙:

```markdown
앱 화면 파일 요청(Static Assets)
→ Cloudflare가 파일을 그대로 응답
...
앱 화면 파일(Static Assets)까지 모두 Worker script가 처리하게 만들면 API 사용량 한도를 불필요하게 쓴다. 따라서 앱 화면 파일은 Cloudflare가 그대로 보내고, 데이터 조회·저장·수정·삭제와 이미지 업로드 요청(Worker API)만 Worker가 처리하게 둔다.

# 4. 비용/운영 리스크

- 앱 화면 파일(Static Assets) 요청은 무료·무제한이고, 화면 파일을 Cloudflare에 올려두는 저장 비용도 따로 없다.
...
비용 용어를 쉽게 풀면 다음과 같다. R2의 10 GB-month는 한 달 평균 10GB까지 이미지 파일을 저장할 수 있다는 뜻이다. Class A operation은 업로드·삭제·파일 목록 보기 같은 관리 작업이고, Class B operation은 썸네일 보기처럼 파일을 읽는 작업이다. egress bandwidth 요금 없음은 사용자가 이미지를 볼 때 R2에서 브라우저로 나가는 다운로드 트래픽 비용을 따로 받지 않는다는 뜻이다.
...
따라서 이미지 preview를 1차 기능으로 제대로 넣는다면 D1 단독은 맞지 않고, R2 같은 파일 저장소(object storage)가 필요하다.
...
비유하면 D1은 목록표(database)이고 R2는 파일함(object storage)이다. 목록표에는 `previews/item-123/preview.webp` 같은 이미지 위치값(`image_key`)만 적고, 실제 이미지는 파일함에 둔다.
...
D1 백업(dump)은 프롬프트·태그·Workflow 같은 목록표 데이터를 저장하는 일이고, R2 백업(sync)은 JPG/PNG 썸네일 이미지 파일을 복사하는 일이다. 둘 중 하나만 백업하면 완전한 복구가 안 되므로, D1 dump와 R2 sync를 한 백업 스크립트로 묶는다.
